...
class Blockchain{
  ...
  async validateBlock(blockHeight){
   ...
   async validateChain(){
    ...
    getHeight(key){
      return new Promise(function(resolve, reject){
        db.get(key, function(err, value){
          ...
          if (parseInt(key) > 0) {
            value.star.storyDecoded = new Buffer(value.star.story, 'hex').toString()
          }
    ...
    // Get blocks by hash value from LevelDB with hash
    getBlockHash(hash) {
     let block;
     return new Promise(function(resolve, reject){
      db.createReadStream().on('data', function(data){    
    ...
    // Get blocks by address from LevelDB with address
